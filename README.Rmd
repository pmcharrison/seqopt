...
```
# seqopt v`r as.character(packageVersion("seqopt"))`
...
`seqopt` is an R package for finding optimal sequences with dynamic programming.
Given a list of timepoints and corresponding lists of possible states, 
`seq_opt()` efficiently finds an optimal state sequence that minimises an
